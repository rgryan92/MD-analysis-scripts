...
path2='BM_March2017_HONOretr_'
test = 't1'
location = path2+test+'\\'

# Date format for axes: (year, month, day, hour)
startdate = datetime.datetime(2017, 3, 6, 6)  
...
dates_list = np.arange(20170302, 20170320, 1)
df_list = []
#%%
...
all_profiles_df  = all_profiles_df[all_profiles_df['err_'+tg+'_VCD']<2e15]     
all_profiles_df  = all_profiles_df[all_profiles_df['err_surf_vmr']<0.0006]     
...
ax.errorbar(x,y,yerr=err, fmt='k.', label= tg+' VCD')
...
fig = plt.figure(figsize=(60,3))
ax=fig.add_subplot(111)
ax.errorbar(x,y1,yerr=err1, fmt='k.', label=tg+' VMR')
ax.set_xlim(startdate, enddate)
ax.set_ylim(0, 0.03)
...
ax2.grid(b=None)
fig.savefig(path+location+test+'_'+tg+'_vcd_and_svmr_'+date+'.pdf', 
                    bbox_inches='tight')
#%%
...
for d in dates_list:
    ...
    prdf = pd.read_csv(path+location+date+'/general/all_'+tg1+'_vmr_prof_'+date+'.dat',
...
keptticks = xticks[::int(len(xticks)/31)]
xticks = ['' for a in xticks]
xticks[::int(len(xticks)/31)] = keptticks
...
sb.heatmap(all_vprof_df[::-1], cmap=cm.RdYlBu_r, ax=ax, vmin=0, vmax=0.0006,
