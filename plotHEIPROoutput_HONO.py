...
ss = [(((y[i]/1e15)-(y1[i])/1e15)**2) for i in np.arange(len(y))]
...
plt.text(10, 5e15, 'RMS = '+str(round(rms,5)), size=fontsize)
...
dextfile = pd.read_csv(path+location+date+'/general/HCHO_retrieval_'+date+'.dat',
                     delim_whitespace=True, parse_dates=[['Date', 'Time']])
dextp = dextfile.plot(x='Date_Time', y='HONO_VCD(molec/cm^2)', yerr='err_HONO_VCD', color='green',
                      figsize=(4,3))
fig = dextp.get_figure()
fig.savefig(path+location+test+'_HONO_VCD_'+date+'.png', 
                   bbox_inches='tight')
...
dextp = dextfile.plot(x='Date_Time', y='surf_vmr(ppmv)', yerr='err_surf_vmr', color='darkolivegreen',
                      figsize=(4,3))
fig = dextp.get_figure()
fig.savefig(path+location+test+'_HONOsurf_vmr_'+date+'.png', 
                   bbox_inches='tight')
#%%
